...
include { UNTAR as UNTAR_VAT_INDEX          } from '../../../modules/nf-core/untar'
...
include { VAT_BUILD                         } from '../../../modules/local/vat/build'
...
workflow PREPARE_GENOME {
    ...
    vat_index                // directory: /path/to/vat/index/
    ...
    aligner                  // string: Specifies the alignment algorithm to use - available options are 'star_salmon', 'star_rsem', 'hisat2' and 'vat'
    ...
    //---------------------------------------------------------
    // 14) VAT index -> needs FASTA if built
    //---------------------------------------------------------
    ch_vat_index = Channel.empty()
    if ('vat' in prepare_tool_indices) {
        if (vat_index) {
            if (vat_index.endsWith('.tar.gz')) {
                ch_vat_index = UNTAR_VAT_INDEX ([ [:], file(vat_index, checkIfExists: true) ]).untar.map { it[1] }
                ch_versions  = ch_versions.mix(UNTAR_VAT_INDEX.out.versions)
            } else {
                ch_vat_index = Channel.value(file(vat_index, checkIfExists: true))
            }
        }
        else if (fasta_provided) {
            ch_vat_index = VAT_BUILD(
                ch_fasta.map { [ [:], it ] }
            ).index.map { it[1] }
            ch_versions = ch_versions.mix(VAT_BUILD.out.versions)
        }
    }
    ...
    vat_index        = ch_vat_index              // channel: path(vat/index/)
