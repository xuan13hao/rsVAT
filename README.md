...
8. Choice of multiple alignment and quantification routes (_For `STAR` the sentieon implementation can be chosen_):
   ...
   4. [`VAT`](https://github.com/xuan13hao/VAT) -> **NO QUANTIFICATION**
...
> **Warning**
> Quantification isn't performed if using `--aligner vat`. The VAT route produces genomic alignments and reuses the pipeline's BAM processing and QC steps, but it does not currently produce gene or transcript expression estimates.
...
This fork adds VAT as an additional genomic alignment backend. VAT can be selected with:

```bash
--aligner vat
```

The VAT route builds or receives a VAT nucleotide database, aligns RNA-seq FASTQ reads, converts VAT SAM output to BAM with SAMtools, and then reuses the existing BAM sort, index, statistics, coverage and MultiQC steps.

<!-- The VAT commands used by the pipeline are:

```bash
VAT makevatdb --in <reference.fa> --dbtype nucl -p <threads>
VAT nucl short RNAseq -d <ref.fa> -q <reads.fq> -o <out.sam> -f sam -p <threads>
```

By default, the VAT Singularity/Apptainer image is expected at:
...
<!-- Useful VAT-specific parameters:

- `--vat_index`: Path to a pre-built VAT nucleotide database directory or `.tar.gz` archive. If omitted, the database is built from `--fasta`.
- `--vat_container`: Path to the VAT Singularity/Apptainer image. Defaults to `bin/VAT_latest.sif`.
- `--extra_vat_build_args`: Extra arguments appended to `VAT makevatdb`.
- `--extra_vat_align_args`: Extra arguments appended to `VAT nucl short RNAseq`.

The current VAT command accepts one FASTQ input via `-q`. For paired-end samples, this implementation concatenates the sample FASTQ inputs into one temporary FASTQ before calling VAT. If VAT paired-end options are available, the `VAT_ALIGN` module can be updated to pass R1/R2 separately.

For a focused VAT integration guide, see [`README_VAT.md`](README_VAT.md).
