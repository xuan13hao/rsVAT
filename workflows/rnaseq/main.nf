...
workflow RNASEQ {
    ...
    //
    if (!params.skip_alignment && params.aligner == 'vat') {
        FASTQ_ALIGN_VAT (
            ch_strand_inferred_filtered_fastq,
            ch_vat_index.map { [ [:], it ] },
            ch_fasta.map { [ [:], it ] }
        )
        ch_genome_bam          = ch_genome_bam.mix(FASTQ_ALIGN_VAT.out.bam)
        ch_genome_bam_index    = ch_genome_bam_index.mix(params.bam_csi_index ? FASTQ_ALIGN_VAT.out.csi : FASTQ_ALIGN_VAT.out.bai)
        ch_unprocessed_bams    = ch_genome_bam.map { meta, bam -> [ meta, bam, '' ] }
        ch_multiqc_files       = ch_multiqc_files.mix(FASTQ_ALIGN_VAT.out.summary.collect{it[1]})
